...
public class _14_ProteinCodingGeneImport {
	...
	public static void main(String[] args) throws IOException {
		...
		String fileName = "/Users/xMAnton/biodb/protein-coding_gene.txt";
		String line;
    	...
    	OrientVertexType vgene = graph.createVertexType("geneName");
    	vgene.createProperty("symbol", OType.STRING);
        	...
        	*/
        	...
        	if (!entrezId.equals("")) {
        		...
        		if (gene != null) {            		
        			...
        			String entrezSymbol = gene.getProperty("nomenclatureAuthoritySymbol");
                	...
                	if (!uniprotIds.equals("")) {
                		if (uniprotIds.contains("|")) {
                    		...
                    		for (int i=0; i<uniprotId.length; i++) {
                    			it = graph.getVertices("proteinName.name", uniprotId[i]).iterator();
                    			if (it.hasNext()) {
                			...
                			it = graph.getVertices("proteinName.name", uniprotIds).iterator();
                			if (it.hasNext()) {
                				Edge e = it.next().getEdges(Direction.OUT, "refersTo").iterator().next();
                				Vertex protein = e.getVertex(Direction.IN);
		...
		fileName = "/Users/xMAnton/biodb/non-coding_RNA.txt";
